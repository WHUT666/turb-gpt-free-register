...
def _ssl_verify() -> bool:
    """与 core.session 一致：本地 MITM 代理可用 SSL_VERIFY=False。"""
...
def _default_proxy() -> str | None:
    """优先 CODEX_AGENT_PROXY / HTTPS_PROXY；否则取 config.PROXY_POOL 第一个。"""
    for key in ("CODEX_AGENT_PROXY", "HTTPS_PROXY", "HTTP_PROXY", "ALL_PROXY"):
        value = _normalize_proxy(os.getenv(key))
        if value:
            return value
    try:
        ...
        if pool:
            return _normalize_proxy(pool[0])
    except Exception:
        pass
    return None


def _request_kwargs(proxy: str | None = None) -> dict[str, Any]:
    """公共请求参数：TLS 指纹 + SSL 校验 + 可选代理。"""
    kwargs: dict[str, Any] = {
        "impersonate": IMPERSONATE,
        "verify": _ssl_verify(),
        "timeout": 30,
    }
    proxy_url = _normalize_proxy(proxy) or _default_proxy()
    if proxy_url:
...
def get_session_from_cookies(cookies: dict[str, str], proxy: str | None = None) -> dict[str, Any]:
    """
    使用 cookies 调用 /api/auth/session 获取 accessToken 和账号信息。

    :param cookies: chatgpt.com 的 cookies dict
    :param proxy: 可选代理 URL（与注册出口保持一致，避免地区限制）
    :return: session 数据
    """
    r = requests.get(
...
def register_agent(
    ...
    :param proxy: 可选代理 URL（建议与账号注册出口一致）
...
def create_codex_agent_identity(
    ...
    :param proxy: 可选代理；建议与账号注册时出口一致，避免 unsupported_country
    :return: auth.json dict
    """
    # 确保能读到项目 .env 中的 SSL_VERIFY 等开关（CLI 单独跑时也生效）
    try:
    ...
    proxy_url = _normalize_proxy(proxy) or _default_proxy()
    if proxy_url:
        _log("Proxy", f"使用代理: {proxy_url}")
    else:
        _log("Proxy", "未配置代理（直连）", "WARN")
    ...
    agent_runtime_id = register_agent(access_token, public_key_ssh, proxy=proxy_url)
    ...
    if verify_task:
        ...
        try:
            task_id = register_task(access_token, agent_runtime_id, private_key_b64, proxy=proxy_url)
...
def main() -> None:
    ...
    parser.add_argument("--proxy", type=str, help="代理 URL，建议与注册出口一致")
